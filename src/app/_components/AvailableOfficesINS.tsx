...
interface SelectAvailableOffice {
  onClose: () => void; // الدالة لإغلاق المكون
  // onOfficeSelect: (orderId: string) => void;
}
function AvailableOfficesINS({ onClose }: SelectAvailableOffice) {
  ...
  const handleSubmit = async (eve: any) => {
    eve.preventDefault();
    setIsLoading(true);
    const data: any = localStorage.getItem("orderInstitute");
    const orderIns = JSON.parse(data);
    const selectedOffice = eve.target.office.value;

    const {
      userId, instituteBranchId,
      courseId, coursePrice,
      booksFees, instituteFees,
      residencePrice, totalPrice,
      transfer_fees, eduxa_fees,
      convertedPrice, startDate,
      typeOfCourse, numberOfWeeks,
      numberOfWeeks_for_residence,
      airportReception, residence,
      healthInsurance, deposit, coupon } = orderIns


    try {
      const row = JSON.stringify({
        "userId": parseInt(userId),
        "instituteBranchId": parseInt(instituteBranchId),
        ...
        "eduxa_fees": eduxa_fees,
        "convertedPrice": convertedPrice,
        "startDate": startDate,
        "typeOfCourse": typeOfCourse,
        "numberOfWeeks": parseInt(numberOfWeeks),
        "numberOfWeeks_for_residence": parseInt(numberOfWeeks_for_residence),
      ...
      <div className="bg-white rounded-3xl md:w-[665px] p-3 md:p-0">
        ...
        >
            ...
            {createQuote === "stepOne" ?
              <>
                <div className="rounded-lg border border-solid border-zinc-100 md:m-6">
                  ...
                  {error &&
                    <div className="text-red-500 flex gap-1 items-center py-2 text-sm md:text-base px-2">
                      <div>
                ...
                <div className="relative mx-6 mt-4 mb-0 rounded-lg border border-solid border-zinc-100">
                  <div className="box-border flex justify-start items-center p-4 w-full border-b border-solid border-b-zinc-100">
                    <div onClick={() => console.log(availableOffices)}
                  ...
                  {availableOffices.length == 0 ? <div className="flex  w-1/3 mx-auto my-10 justify-center text-center">
                    <Spinner />
                  </div> : <div className="flex flex-wrap flex-col md:flex-row gap-4 p-2 md:p-4 max-sm:gap-4">
                    {availableOffices?.map((office: { id: any, name: string, address: string }) => {
                      return <>
          ...
          {createQuote === "stepThree" ?
            <PaymentDone orderID={oderId} onClose={onClose} /> : ""}
        </div>
      </div>
    </div>
  );
...
export default AvailableOfficesINS;
