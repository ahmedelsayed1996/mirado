...
import NotEnrolledProgram from "./NotEnrolledProgram";
...
import { getReviewUNI } from '../reduxTool-kit/slices/reviewUNISlice';

function RatingUNI({ universityID }: any) {
  const t = useTranslations("Rating");
  const [isOpen, setIsOpen] = useState(false);
  const [showPoppup, setShowPoppup] = useState(false);
  const [reviewAndRating, setReviewAndRating] = useState(null);
  const [reviewsINS, setReviewsINS] = useState(null);
  const [chooseRating, setChooseRating] = useState(false);
  const language = useCurrentLang();
  const dispatch = useDispatch<AppDispatch>();
  const [rating, setRating] = useState<number>(0);
  const reviews = useSelector((state: any) => state.reviewUNI);
    ...
    if (chooseRating) {
      if (tokenMainSite) {
        ...
        try {
          ...
          const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/university-reviews`, {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    dispatch(getReviewUNI({ limt: 20, language, university_id: universityID }))
  ...
  return (
    <div className="flex flex-col self-stretch">
      <div className="w-full text-2xl font-semibold leading-9 text-start text-gray-900 max-md:max-w-full">
      ...
      {!reviews.loading ? <>
              ...
              <div className="flex flex-col w-[73%] max-md:ml-0 max-md:w-full">
                <div className="flex gap-3 mt-8 max-md:flex-wrap max-md:mt-10 max-md:max-w-full">
                  <div className="flex flex-col justify-center text-xs font-medium leading-5 text-center text-gray-900 whitespace-nowrap">
                    {/* <div>5 {t("star")}</div>
                    <div className="mt-2">4 {t("star")}</div>
                    <div className="mt-2">3 {t("star")}</div>
                    <div className="mt-2">2 {t("star")}</div>
                    <div className="mt-2">1 {t("star")}</div> */}
            ...
            <form onSubmit={handleSubmit}>
              <div>
                <textarea
                  ...
                  required
                  id="OrderNotes"
                  className="mt-2 w-full rounded-lg border border-gray align-top shadow-sm sm:text-sm p-2 focus:outline-none"
                  rows={4}
                  placeholder={t('placeHolder')}
                  defaultValue={""}
                  onChange={(e: any) => setReviewAndRating(e.target.value)}
                ></textarea>
        ...
        <div className="flex flex-col items-start self-start mt-3 max-w-full text-sm w-[634px]">
          ...
          <div className="self-stretch mt-3 leading-5 text-start text-gray-900 max-md:max-w-full">
