...
const FavoriteTableProgram: React.FC<Props> = ({ headers, data }) => {

    const [favoritesProgram, setFavoritesProgram] = useState(data);
    ...
    const unFavorite = async (programId: number) => {
        favoritesProgram.filter((item) => item.id !== programId);
        ...
        try {
            ...
            setFavoritesProgram((prev: any) => prev.filter((favorite: any) => favorite?.program?.id !== programId));
        } catch (error: any) {
    ...
    return (
        <div className="overflow-auto w-full max-w-[1150px] mt-4">
            <table className="w-full border-collapse ">
                ...
                <tbody>
                    {favoritesProgram?.map((item: any, index: number) => {
                        return (
                            <tr
                                key={index}
                            ...
                            >
                                ...
                                <td className="py-2 px-4">
                                    {item?.program?.price} {item?.program?.university?.currency}
                                </td>
                                <td className="py-2 px-4">
                                    {item?.program?.program_translations[0].course_language}
                                </td>
                                <td className="py-2 px-4">
                                    {item?.program?.price_period} {item?.program?.program_translations[0].price_period_type}
                                </td>

                                <td className="py-2 px-4">
                                    <div className="flex gap-2 justify-start">
                                        {loadingStates.get(item?.program?.id) ? <div className=" p-1 bg-white rounded border border-solid border-zinc-100 cursor-pointer">
                                        ...
                                        </div> : <div className=" p-1 bg-white rounded border border-solid border-zinc-100 cursor-pointer"
                                            onClick={() => unFavorite(item?.program?.id)}>
...
export default FavoriteTableProgram;
