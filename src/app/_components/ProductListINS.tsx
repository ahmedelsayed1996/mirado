...
function ProductListINS({ props, recommended }: {
  props: number;
  recommended: String;
}) {
  ...
  // const [languageSchools, setLanguageSchools] = useState<Object[]>([]);
  ...
  const handlePageChange = (newPage: any) => {
    if (hasMoreData && newPage !== currentPage) {
      ...
      dispatch(getAllLanguageSchools({ page: newPage, language, recommended: recommended, limt: props, userId: user.id }));
    }
  };
  ...
  //   }
  // }, [currentPage, dispatch, hasMoreData, language, props, recommended, user.id]);


  return (<>
    {languageSchools?.languageSchools?.data?.length > 0 ? languageSchools.languageSchools?.data?.map((languageSchool: any, index: number) => {
      return (
        <div key={index} className="flex flex-col p-2 md:p-5 bg-white rounded-2xl border border-gray border-solid shadow-2xl md:m-5 mb-2">
              ...
              <div className="flex flex-col ml-5 w-[33%] max-md:ml-0 max-md:w-full">
                <div className="flex flex-col grow justify-center max-md:mt-6">
                  <div className="flex overflow-hidden relative flex-col py-2.5 px-2 pl-3.5 w-full aspect-[1.5] max-md:pr-5">
                    <Image
                      src={languageSchool.files?.length === 0 ? "/images/institut.png" : `${process.env.NEXT_PUBLIC_BASE_PHOTO_URL}/${cleanPath(languageSchool.files[0])}`}
                    ...
                    {languageSchool.offer ?
                      <div className="relative justify-center self-start px-2.5 py-2 text-base font-medium tracking-tight leading-6 text-center text-white bg-blue-900 rounded-[111px] max-md:px-5">
                        {languageSchool.offer}% {t("offerIcon")}
                      </div>
                      : ""}
              ...
              <div className="flex flex-col w-[67%] max-md:ml-0 max-md:w-full">
                <div className="flex flex-col mt-6 max-md:max-w-full">
                  <div className="flex gap-4 max-md:flex-wrap">
                    <div className="flex w-1/6 justify-center items-center p-2.5 bg-white rounded-xl border border-gray border-solid h-[60px] lg:h-[125px] relative">
                      <Image
                    ...
                    <div className="flex w-4/6 flex-col my-auto text-start max-md:max-w-full">
                      <div className="self-start text-xl font-medium text-gray-900 line-clamp-1">
                        {languageSchool.name}
                      </div>
                      <div className="flex gap-1 mt-3 text-base tracking-wide leading-6 text-zinc-500 max-md:flex-wrap">
                        ...
                        {/* {languageSchool?.countries?.map((countr: any, index: number) => {
                          return (
                            <div key={countr.id} className="flex gap-1">
                              <div className="flex-1 line-clamp-1">
                                {countr.name}
                                {index !== languageSchool.countr?.length - 1 && " - "}
                              </div>
                            </div>
                          )
                        })} */}
          ...
          <div className="flex gap-0 justify-between mt-3.5 max-md:flex-wrap">
            <div className="flex gap-5 justify-center md:px-2 mt-3.5 text-sm leading-6 text-start">
              ...
              <div className="flex gap-1.5 justify-center py-1.5 rounded-2xl">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="font-medium text-[#ACB5BB] line-clamp-1">{languageSchool?.rating ? languageSchool?.rating?.toFixed(1) : "N/A"}</div>
                </div>
              </div>
            </div>
            <div className="flex gap-2.5 my-auto max-md:flex-wrap max-md:max-w-full items-center">
              ...
              {loadingStates.get(languageSchool.id) ? <Spinner /> :
                <>
                  <Link
                    href={`/${pathname.slice(1, 3)}/language-schools/${languageSchool.id}`}
                    onClick={() => setLoadingStates((prev) => new Map(prev).set(languageSchool.id, true))}
                  >
                    <button className="flex-1 justify-center px-2 py-2  text-white bg-primary rounded-xl max-md:px-5 border hover:border hover:border-primary hover:text-primary hover:bg-white transition-all duration-300">
                      {t("cardButtonins")}
                    </button>
                  </Link>
                  <div className="flex justify-center items-center px-4 py-2 rounded-2xl border border-gray hover:border-primary border-solid cursor-pointer" onClick={() => {
