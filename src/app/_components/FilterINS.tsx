...
import ProductINS from "./ProductINS";
import ProductListINS from "./ProductListINS";

const FilterINS: React.FC = () => {
  const [showFilter, setShowFilter] = useState<boolean>(true);
  const [card, setCard] = useState<boolean>(true);
  const [recommend, setRecommended] = useState<string | null>("all");
  ...
  const getNumberOfStars = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/institute-reviews/ratings/summary`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <div className="flex flex-col w-full ">
      <div className="flex justify-between py-1 mt-8 w-full text-start max-md:flex-wrap max-md:max-w-full">
        <div className="my-auto text-xl font-medium leading-6 text-primary items-center">
          <span> {f("filterResults")} </span>(
          <span className="text-primary text-base">
            {languageSchools?.languageSchools?.total}
          </span>
          )
        </div>
        <div className="flex gap-2.5 max-md:flex-wrap">
          <div className="flex gap-0 justify-center p-1.5 text-sm font-medium bg-stone-50 rounded-[111px]">
            <button
              onClick={showList}
              className={`flex flex-1 gap-1.5 justify-center px-2 py-3 ${
                card
                  ? "bg-amber-500 text-white "
                  : "text-zinc-500 bg-transparent"
              } rounded-[111px]`}
            >
              <Image src="/icons/grid.svg" width={20} height={20} alt="grid" />
              <div> {f("cards")} </div>
            </button>
            <button
              onClick={showCard}
          ...
          <button
            ...
            className={`flex gap-1 justify-center px-2 py-3 my-auto text-sm leading-5 bg-white rounded-lg border border-solid border-zinc-100 text-zinc-500 focus:bg-amber-500 focus:text-white cursor-pointer ${
              showFilter ? "active" : ""
            }`}
          >
            <div>
      ...
      >
            ...
            <div className="flex flex-col justify-center px-5 pt-5 pb-9 mx-auto w-full bg-white rounded-xl border border-gray border-solid shadow-2xl">
              ...
              <div className="space-y-4">
                <details
                  ...
                  open
                >
                  <summary className="flex cursor-pointer items-center justify-between gap-1.5 rounded-lg bg-secondColor p-4 text-gray-900">
              ...
              <div className="space-y-4 ">
                ...
                >
                  <summary className="flex cursor-pointer items-center justify-between gap-1.5 rounded-lg bg-secondColor my-5 px-4  py-2 text-gray-900 ">
                    <h2 className="font-medium">{s("dropDown1")}</h2>
                    <svg
                      className="size-5 shrink-0 transition duration-300 group-open:-rotate-180"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
        ...
        <div
          className={`flex flex-col transition-all duration-300 ${
            showFilter ? "md:w-7/12 lg:w-9/12" : "w-full"
          } `}
        >
          <div className="transition-all duration-300 flex flex-col grow max-md:mt-10 ">
            <div className="">
              {card ? (
                <ProductINS props={20} bg="transparent" recommended="" />
              ) : (
                <ProductListINS props={20} recommended="" />
              )}
            </div>
          </div>
        </div>
